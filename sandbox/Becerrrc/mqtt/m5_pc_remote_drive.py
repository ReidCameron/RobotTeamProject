...
Authors: David Fisher and Raymond Becerra.
...
def main():
    ...
    forward_button['command'] = lambda: human_pressed_forward(mqtt_client, left_speed_entry, right_speed_entry)
    root.bind('<Up>', lambda event: human_pressed_forward(mqtt_client, left_speed_entry, right_speed_entry))
...
def human_pressed_forward(mqtt_client, left_speed_entry, right_speed_entry):
    L = left_speed_entry.get()
    R = right_speed_entry.get()
    print('L =', L)
    print('R =', R)
    mqtt_client.send_message('forward', [int(L), int(R)])
